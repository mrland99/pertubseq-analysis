...
class CellPopulation:
    ...
    def __init__(self, matrix, cell_list, gene_list, source='arrays', normalized_matrix=None,
    ...
    def from_hdf(cls, filename):
        ...
        return cls(matrix, cell_list, gene_list, source=filename, normalized_matrix=normalized_matrix,
    ...
    def from_file(cls, directory, genome=None, filtered=True, raw_umi_threshold=2000):
        ...
        if os.path.isdir(os.path.join(directory, os.path.normpath("outs/filtered_gene_bc_matrices"), genome)):
            if filtered:
                ...
                matrix_directory = os.path.join(directory, os.path.normpath("uprPerturbFiles"))
        else:
            if filtered:
                matrix_directory = os.path.join(directory, os.path.normpath("outs/filtered_gene_bc_matrices_mex"),
                                                genome)
            else:
                matrix_directory = os.path.join(directory, os.path.normpath("uprPerturbFiles"))"""
        ...
        print('Loading digital expression data: {0}...'.format(os.path.join(matrix_directory, "matrix.mtx.txt")))
        ...
        matrix = mmread(os.path.join(matrix_directory, "matrix.mtx.txt"))
        ...
        if filtered:
            cell_list = pd.merge(cell_barcodes, guide_identities, left_on='cell_barcode', right_index=True,
                                 how='left').set_index('cell_barcode')
        else:
            cell_list = pd.merge(cell_barcodes.iloc[ind], guide_identities, left_on='cell_barcode', right_index=True,
                                 how='left').set_index('cell_barcode')
        ...
        cell_list['single_cell'] = (cell_list['number_of_cells'] == 1) & (cell_list['good_coverage']) & (
            ~(cell_list['guide_identity'] == '*'))
        cell_list['UMI_count'] = matrix.sum(axis=1)

        return cls(matrix, cell_list, gene_list, source=directory)

    def where(self, cells=None, genes=None, normalized=False, gene_names=False, densify=True, return_query_str=False,
              dropna=False, **kwargs):
    ...
    def fit_transform(self, transformer, y=None, cells=None, genes=None, normalized=False, prefix=None,
    ...
    def plot(self, data, clusters=None, traits=None, gene=None, normalized=True, cluster_subset=None,
             cluster_size_threshold=0, cm=None, randomize_cmap=False, s=10, alpha=1, auxiliary_matrix=None,
             no_axes=True, no_cbar=False):
    ...
    def plots(self, data, properties, types=None, randomize_cmap=False, normalized=True, auxiliary_matrix=None, s=10,
              alpha=1, no_axes=True, tight_layout=True):
        ...
        for i, prop in enumerate(properties):
            ...
            if (types[i] == None) or (types[i] == 'discrete'):
                ...
                self.plot(data, gene=prop, normalized=normalized, auxiliary_matrix=auxiliary_matrix, s=s, alpha=alpha,
                          no_axes=no_axes)
                plt.xlabel(prop)
...
class MeanPopulation(CellPopulation):
    ...
    def expression_table(self, genes, cluster_size_thresh=0, sort=True, linkage=None, optimal_ordering=False, **kwargs):
        ...
        if linkage is not None:
            ...
            if self.normalized_matrix is not None:
                ...
                Z = self.cluster_genes(genes, method='ward', metric='euclidean', normalized=False,
    ...
    def expression(self, genes, cluster_size_thresh=0, cluster_order=None, sort=True, linkage=None,
        ...
        data = self.expression_table(genes, cluster_size_thresh=cluster_size_thresh, sort=sort, linkage=linkage,
                                     optimal_ordering=optimal_ordering)
        n_clusters = len(data['weight'].columns)

        # GeneCard links for gene names
        data['gene_name'] = data['gene_name'].apply(
            lambda x: '<a href="http://www.genecards.org/cgi-bin/carddisp.pl?gene={0}">{1}</a>'.format(x, x))
    ...
    def expression_heatmap(self, genes, cluster_size_thresh=0, cluster_order=None, linkage=None, normalized=False,
                           **kwargs):
        ...
        """
        # get cluster statistics
        data = self.expression_table(genes, cluster_size_thresh=cluster_size_thresh, cluster_order=cluster_order,
                                     sort=False)
        if normalized:
...
def correlation_heatmap(pop, gene_list, corr_method='pearson', normalized=False, linkage=None, robust=True, method=None,
                        metric=None, **kwargs):
